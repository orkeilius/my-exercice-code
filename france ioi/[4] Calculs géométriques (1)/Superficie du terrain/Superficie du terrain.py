...
for loop in range(int(input())):
    ...
    # calcul connected area
    squareIter = square(*[int(i) for i in sys.stdin.readline().split()])
    x = max(0, min(squareIter.x2, mainSquare.x2) -
            max(squareIter.x1, mainSquare.x1))
    y = max(0, min(squareIter.y2, mainSquare.y2) -
            max(squareIter.y1, mainSquare.y1))

    # remove connected area from total
    space -= x * y
